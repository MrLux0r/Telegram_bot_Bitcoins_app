...
def update_user(user_id: int, user: pydantic_models.User_to_update = fastapi.Body(), current_user: Admin = Depends(get_current_user)): # используя fastapi.Body() мы явно указываем, что отправляем информацию в теле запроса
    """Обновляем юзера"""
    if user_id == user.id:
...
@crud.db_session
def delete_user(user_id: int = fastapi.Path(), current_user: Admin = Depends(get_current_user)): # используя fastapi.Path() мы явно указываем, что переменную нужно брать из пути
    """
    Удаляем юзера
    :param user_id:
    :return:
...
@crud.db_session
def create_transaction(user_id, transaction: pydantic_models.Create_Transaction, current_user: Admin = Depends(get_current_user)):
